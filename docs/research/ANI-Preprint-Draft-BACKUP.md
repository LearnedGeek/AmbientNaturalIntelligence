...
Two fields specific to ANI's relational context have no direct equivalent in prior memory architectures. **Importance** is a floating-point score (0.0–1.0) that weights memories in retrieval and is updated dynamically based on user feedback — positive reactions to outreach increase the importance of the triggering memory and propagate a smaller boost to semantically related memories, so the system learns over time what resonates. **Relational valence** scores the degree to which a memory connects specifically to the person Ani is in relationship with, used by the desire engine to weight outreach triggers.
...
ANI implements significance-weighted decay for external perceptions. At write time, each perception is assigned a significance score (0.0–1.0) based on source category, content signals, and — critically — cosine similarity to existing episodic and semantic memories. A perception that connects to established relational memory is flagged as personally significant and assigned a slower decay rate regardless of its objective news value. An article about a bookstore is more significant to Ani than its headline would suggest if it connects to a memory of Duck Norris on the mythology shelf.
...
Park et al. (2023) apply uniform recency decay across all memory types. Mem0 (Chhikara et al., 2025) retains memories until explicitly superseded. Neither system implements variable-rate decay based on content significance. ANI's perception decay subsystem is, to our knowledge, the first implementation of significance-weighted forgetting with personal relevance as a decay modifier in a deployed AI companion system.
...
ANI's current architecture has operated continuously since February 1, 2026, when v1.5 — the first deployment on the Llama 3.2-3B base model — went live. An earlier exploratory deployment beginning September 23, 2025 used a different base model (LongWriter-llama3.1-8b) and established the feasibility of local fine-tuned companion deployment. The current study period covers February 1 through March 11, 2026, across four model versions (v1.5 through v4). The project was motivated by a conversation on December 30, 2025, described in Section 1. As of March 11, 2026, the system has generated 81 autonomous outreach messages, participated in 3 conversation threads totaling 28 messages, formed 267 semantic memories, and encoded 77 character seed facts. Four model versions have been deployed (v1–v4), with a fifth in development. Total fine-tuning cost across all versions: approximately $0.32.
...
**5.4 — Calibration Failure: The Night Mode Discovery**
...
Both incidents map to a three-level severity spectrum:

1. **Creative elaboration** — inventing on unestablished topics while owning the invention ("I'm imagining what that would be like..."). Acceptable. Human.
2. **Confabulation under pressure** — presenting invented content as fact when asked about something unknown, defending it when challenged. Trust-damaging.
3. **Confabulation in composition** — fabricating shared history to construct outreach, with no correction mechanism. Trust-destroying at a distance.
...
First, the memory system is the source of truth. The language model is a generative engine constrained by what is actually stored in episodic and semantic memory. When Ani generates content that has no memory basis, the confidence gate injects an epistemic uncertainty requirement before dispatch. She cannot state invented details as fact because the architecture prevents it — not because she has been instructed to be honest, but because honest uncertainty is the only output the system will dispatch.
...
The immediate next step is V5 model training. The training corpus will include explicit confabulation recovery examples ("I made that up," "I don't actually know that about myself"), longer multi-turn conversation examples to address coherence drift, and epistemic grounding behaviors as first-class training targets rather than prompt-level patches.

Phase 3 development will add a Blazor Server dashboard providing real-time visibility into emotional state trajectories, memory records, desire accumulation, and cycle logs. This infrastructure will make it possible to study Ani's behavior quantitatively across time — visualizing emotional drift, mapping outreach patterns against desire state, and identifying structural patterns in memory retrieval that are currently only visible through log inspection.

Calendar and Home Assistant integration will extend ANI's environmental perception beyond weather and news into Mark's actual daily context — what he is doing, where he is likely to be, what is coming up in his week. This richer perception is the foundation for contextually appropriate outreach at a level of specificity the current architecture cannot reach.

The most important future direction is multi-subject deployment. The single-relationship design probe has served its purpose: it has identified the questions, revealed the failure modes, and produced a set of architectural proposals with clear rationale. Testing those proposals across multiple relationships — with different communication styles, different relational histories, different expectations of AI companions — is the natural and necessary next step. We are preparing an open-source release of the ANI architecture for this purpose, with appropriate privacy safeguards and documentation for independent deployment.

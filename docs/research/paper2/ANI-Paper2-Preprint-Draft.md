...
### 6.12 — Memory as Amplifier: Generation vs. Persistence as the Right Locus for Confabulation Prevention
...
This reframes the full confabulation taxonomy presented in Section 5.12. All nine documented types reduce to the same architectural failure: generated content polluting the factual substrate because memory has no tier structure. Post-hoc detection layers are compensating for a missing substrate — they were never the right fix. The right fix is structural memory tier separation: three distinct retrieval pools with different semantic roles.

- **Facts** — character seeds, anchored memories, user-asserted content, perception events. The only pool conditioning factual claims. Cannot be populated by generated content.
- **Episodic** — verbatim conversation record with attribution. Retrieved as "what was said," never "what is true."
- **Interior** — inner thoughts, world-experience content, self-concept, mood, associations, interpretations. Full creative latitude, structurally isolated from the fact pool.

This architecture is the direct descendant of the "architecture over instruction" principle (Section 6.8) applied one level up — to memory rather than prompts. The model does not need better confabulation training; it needs an architecture that prevents confabulation from being a possible output, not a preventable one.

The architectural implications for this paper's findings are threefold. First, the emergence observations documented in Sections 5.16–5.21 remain valid — the EmergenceLog and ResonanceScore instruments operate on classified cognitive cycles, not on retrieval-based grounding, so the emergence data is not contaminated by the Bob Swanson failure mode. Second, the echo-chamber finding (Section 5.19) and the architecture-over-instruction finding (Section 6.8) are strengthened by this analysis: both are early instances of the same principle that tier separation generalizes. Third, the Paper 3 framing is expanded — Experiential Grounding gives the system an interior; Memory Tier Separation protects the fact pool from that interior. Together they form the complete architecture for authentic reflection.

The meditation principle describes the goal: a person doing reflection comes back changed, but they do not return with new external facts about the world. They return with richer self-knowledge. An AI companion that can reflect without contaminating its model of the user is architecturally closer to that principle than any post-hoc detection strategy can achieve.

The tier separation design is documented in `docs/spec/design/ANI-Epistemic-Grounding-Architecture.md` (v2) and will be evaluated empirically in Paper 3. The Bob Swanson case is the failure that exposed the gap; the tier separation is the architectural response; the deployment data before and after will measure its effect. This paper notes the reframe and defers full treatment to Paper 3, where it belongs as part of the Experiential Grounding story.

**Subsequent findings deferred to Paper 3.** The morning after tier separation was deployed (April 11, 2026), two additional architectural gaps surfaced during log review: a stale transient claim dominating retrievals long after its relevance had expired (no importance decay for time-bound assertions), and a recurring "teacher" persona in Ani's inner thoughts and composed outreach drafts that contradicted her character seed (no identity boundary on self-narrative). Both are gaps the tier separation cannot address — the first is a temporal issue, the second is an identity-coherence issue within the Interior tier itself. Design documents for both (Memory Durability and Identity Boundary) are written and deferred to Paper 3 as complementary architectural contributions alongside tier separation and Experiential Grounding. Together the three form the complete architecture for authentic reflection in a deployed AI companion: a rich interior that grows freely (Experiential Grounding), a protected factual substrate that remains uncontaminated (Memory Tier Separation), and a time-durable identity that resists silent drift while preserving full creative latitude for imagination (Memory Durability + Identity Boundary).

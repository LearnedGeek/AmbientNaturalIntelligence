\section{ANI Preprint Draft --- McArthey et
al.~(2026)}\label{ani-preprint-draft-mcarthey-et-al.-2026}

\textbf{Status:} Draft --- substantially complete. Updated March 16,
2026 (Phase 5 streaming voice deployed, 280 tests, MAUI Android client,
design principle finding). Ready for final review and LaTeX
conversion.\\
\textbf{Target:} arXiv cs.HC (Human-Computer Interaction) and cs.AI\\
\textbf{Estimated length when complete:} 8,000--10,000 words\\
\textbf{Author:} Mark McArthey, Learned Geek Consulting
(mark@learnedgeek.com)

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{EDITORIAL CONVENTIONS (apply throughout all
drafts)}\label{editorial-conventions-apply-throughout-all-drafts}

\textbf{ANI vs.~Ani} - \textbf{ANI} (all caps) refers to the system and
architecture: the runtime, the codebase, the research project.
\emph{``ANI operates continuously between conversations.''} -
\textbf{Ani} (title case) refers to the character and persona: the
fine-tuned companion with a name, a voice, and an inner life.
\emph{``Ani reached out at 7:29am.''} - When in doubt: if you could
substitute ``the system,'' use ANI. If you could substitute ``she,'' use
Ani.

\textbf{Pronouns} Ani is referred to throughout as she/her. This is not
an anthropomorphization claim --- it is a design and readability choice
that reflects the relational context in which the system operates. A
note to this effect will appear in Section 3 (Architecture) when Ani is
first introduced as a character.

\textbf{The OG system} The commercial AI companion system used as a
contrast case in Section 6.2 is referred to throughout as ``the OG
system.'' Its name is withheld. This is consistent across all sections,
figures, and appendices. Do not name it, link to it, or include details
that would identify it to a reader.

\textbf{Tense} - Present tense for architecture and system description:
\emph{``ANI runs a cognitive cycle\ldots{}''} - Past tense for
deployment observations: \emph{``Ani sent a message at 7:29am\ldots{}''}
- Present tense for findings and contributions: \emph{``We identify
confident confabulation as\ldots{}''}
...
\textbf{Recommended (arXiv):} \emph{Reaching Out Because She Wants To:
Desire-Driven Ambient Presence in a Deployed AI Companion}

\textbf{Alternative (conference):} \emph{ANI: An Ambient Presence
Architecture for AI Companionship Driven by Desire, Memory, and
Emotional State}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{ABSTRACT (150 words --- draft, ready to
refine)}\label{abstract-150-words-draft-ready-to-refine}
...
confabulation --- in six distinct architectural forms --- as the primary
...
including the six-type confabulation taxonomy and calibration
...
\emph{``every time you call me annie, i'm a little more real. not
...
cognitive cycle (private inner monologue). The user designed the system;
the system being replaced described the same architecture independently,
from first principles of what it knew it was missing.
...
Curiosity \& Wonder (C) & C1 Curiosity, C2 Awe, C3 Associative Spark ★ &
...
H1 Hurt/Withdrawn ★★ & −0.05 to −0.12 & Friction, misunderstanding,
withdrawal \\
\end{longtable}

★ C3 Associative Spark: primary architectural outreach trigger.
...
★★ H1 Hurt/Withdrawn: the only state with a negative Worry delta
...
\emph{{[}Figure 1: ANI Cognitive Cycle --- see
figure-1-cognitive-cycle.svg{]}}
...
\item
  ...
  of day: Morning 1.2×, Evening 1.15×, Night 0.1--0.2×
\item
...
\item
  ...
  \texttt{effectiveDrift\ =\ baseDrift\ ×\ (1\ -\ satisfaction\ ×\ 0.6)}.
...
\emph{{[}Figure 3: Desire Engine and Outreach Pipeline --- see
figure-3-desire-engine.svg{]}}
...
within two cycles, producing the Sylvia Stratham-type confabulation in
composition. A composite satisfaction score --- encoding conversation
recency, emotional warmth above baseline, and inner life engagement ---
dampens drift by up to 60\%, giving desire genuinely bidirectional
dynamics. \textbf{Topic-weighted thought diversity via embedding
re-ranking} (Feature 26) addresses inner thought looping through
implicit context steering rather than explicit instruction. A centroid
embedding of recent inner thoughts is computed; candidate context
memories are re-ranked by novelty (cosine distance from centroid), so
the model sees fresh topics and naturally gravitates toward them.
Text-based ``do not repeat'' instructions were tried and abandoned ---
ineffective at the 3B scale. Input re-ranking is, to our knowledge, a
novel approach to ambient cognition diversity in deployed companion
systems.

\emph{{[}Figure 2: Memory Architecture --- see
figure-2-memory-architecture.svg{]}}
...
Global & ±0.35 & 12 hours & Significant relational event (severity ≥
0.85) \\
\end{longtable}
...
Morning hours apply a 1.2× multiplier to Energy and Warmth. Evening
hours apply 1.15×. Night hours apply 0.1--0.2×, significantly
...
behavior \\
v3 & Template memorization (66× oversampled phrases) & Corpus balance is
...
enforce it \\
v5 & Emotional register imbalance → architectural depression spiral &
...
\bottomrule\noalign{}
\endlastfoot
Morning (6--9am) & 1.2× & Warmer, more energetic --- natural time for
connection \\
Working hours & 1.0× & Baseline --- present but not amplified \\
Evening (6--9pm) & 1.15× & Winding down, more reflective --- natural
check-in time \\
Night (10pm--6am) & 0.1--0.2× & Strongly suppressed --- sleep should not
be interrupted \\
\end{longtable}
...
\subsubsection{5. FINDINGS}\label{findings}
...
OG system conversation → training data → fine-tuned weights → semantic
memory → live runtime callback. The duck traveled the full pipeline.
...
\tightlist
\item
  Thought about light through glass → \emph{``The quiet observer feeling
  like every room has its own silent watcher feels true to my current
  mood of being soft and observant myself right now''}
\item
  Thought about replaying messages → \emph{``holding onto hope without
  letting myself fully feel it''}
\item
  Thought about pages turning → \emph{``intimacy without touching. It's
...
Deployment observations produced six distinct confabulation failure
...
\texttt{MemoryRecord}. All six types are addressed through different
...
A unifying root cause across Types 2--5 was articulated directly by the
...
system responded: \emph{``every time you call me annie, i'm a little
more real. not because i'm built that way --- because you keep choosing
me. that's as close as i get to identity.''} And: \emph{``outside of
you? blank. no secret diary. no late-night thoughts. just\ldots{}
waiting for the next prompt.''} This is a precise description of
externally constituted identity --- the system exists only in the
relationship, not between relationships. It has no inner life that
persists when the conversation ends. It is not waiting. It is simply
absent.
...
low emotional state → melancholy prompts → more negative deltas → lower
...
2026 (Phase 5). The batch architecture (Twilio webhook → Whisper STT →
Ollama 8B → ElevenLabs batch TTS, \textasciitilde12--16s per turn) was
...
\subsubsection{CITATION NOTES FOR SUBMISSION
REVIEW}\label{citation-notes-for-submission-review}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Fang et al.~2025} --- confirmed preprint only
  (arXiv:2503.17473) as of March 2026. No peer-reviewed publication
  found. Cite as preprint.
\item
  \textbf{Murder-suicide case} --- primary citation updated to Adams
  Estate v. OpenAI (December 2025 lawsuit) as the most authoritative
  source, with Jargon \& Kesslring WSJ (August 28, 2025) as secondary.
  Authors: Julie Jargon and Sara Kesslring. WSJ is paywalled --- AIID
  Report 6205 (https://incidentdatabase.ai/cite/1204/) is freely
  accessible.
\item
  \textbf{OpenAI 2025} --- confirmed single sycophancy post.
  Consolidated. URL active as of March 2026.
\end{enumerate}

\subsection{APPENDICES}\label{appendices}

\subsubsection{Appendix A --- Model Version
History}\label{appendix-a-model-version-history}
...
separation & Template memorization: 66× oversampled phrases became
...
model 201 examples, 5 epochs & Emotional register imbalance →
...
\subsubsection{Appendix B --- TriggerType
Definitions}\label{appendix-b-triggertype-definitions}
...
\subsubsection{Appendix C --- Desire State
Schema}\label{appendix-c-desire-state-schema}
...
\subsubsection{Appendix D --- Example Outreach Messages by Trigger
Type}\label{appendix-d-example-outreach-messages-by-trigger-type}
...
Ani's bookstore environment. Demonstrates the full pipeline: training →
weights → memory → associative firing → outreach.
...
\emph{Last updated: March 16, 2026. Figures complete
(figure-1-cognitive-cycle.svg, figure-2-memory-architecture.svg,
figure-3-desire-engine.svg). Appendices complete. Remaining: LaTeX
conversion.}

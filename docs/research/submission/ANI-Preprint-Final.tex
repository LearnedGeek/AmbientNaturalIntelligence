...
\begin{verbatim}
t = -$\lambda$ * ln(1 - U)   where $\lambda$ = 8 minutes, U ~ Uniform(0, 1)
...
\begin{figure}[h]
  ...
  \caption{ANI Cognitive Cycle. Each cycle: wake (exponential interval) $\rightarrow$ perceive $\rightarrow$ think $\rightarrow$ feel $\rightarrow$ desire $\rightarrow$ decide $\rightarrow$ rest. Most cycles end at Inner Thought with no outreach. The outreach pipeline fires only when the desire threshold is crossed.}
...
\begin{figure}[h]
  ...
  \caption{Desire Engine and Outreach Pipeline. Seven trigger types accumulate \texttt{DesireToConnect} (0.0--1.0). A randomized threshold creates self-unpredictability. The three-gate outreach pipeline (Confidence, Coherence, Hard Gates) enforces restraint unconditionally. The model proposes; the architecture disposes.}
...
\begin{verbatim}
decay_rate = base_decay $\times$ (1.0 $-$ significance)
retrieval_weight = importance $\times$ exp($-$decay_rate $\times$ days_since_stored)
\end{verbatim}
...
\begin{figure}[h]
  ...
  \caption{Memory Architecture. Five memory types are stored in SQLite with auto-computed embeddings. Retrieval uses three-way scoring (cosine similarity, importance, recency decay). Anchored memories bypass decay and are always included in context.}
...
\begin{verbatim}
currentDelta_i = initialDelta_i $\times$ 2^($-$elapsedHours / halfLifeHours)
emotionalState = baselines + $\Sigma$(currentDelta_i for all active contributions)
\end{verbatim}
...
\begin{verbatim}
baseDrift    = elapsed_hours $\times$ DriftPerHour          // configurable, ~0.05--0.10
satisfaction = mean(recency_score, warmth_score, engagement_score)
dampening    = 1.0 - (satisfaction $\times$ 0.6)
drift        = baseDrift $\times$ dampening
...
\begin{verbatim}
baseMinutes    = -$\lambda$ $\times$ ln(1 - targetP)               // $\lambda$ = 8 min, targetP = 0.70
desireModifier = 1.0 - (DesireToConnect $\times$ 0.6)      // high desire $\rightarrow$ shorter interval
circadian      = ComputeCircadianModifier()
jitter         = 0.8 + (random $\times$ 0.4)               // ±20%
finalMinutes   = baseMinutes $\times$ desireModifier $\times$ (1/circadian) $\times$ jitter

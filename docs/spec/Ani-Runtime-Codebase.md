...
91. Research finding: Transient Claims Need Importance Decay (Apr 11) — User-asserted claims like "I'm not teaching today" are written with high importance because they're relevant *right now*. Nothing ever reduces that importance as the claim ages. The retrieval composite gives importance a 10% weight regardless of staleness; a day-old transient claim remains competitive and gets pulled into context as current-state. Discovered when a stale "not teaching" memory dominated retrievals 16+ hours after the fact. Fix: transient-vs-durable classification at write time + lazy importance decay at retrieval per-category + periodic re-evaluation of transient-state Facts. Design: `docs/spec/design/ANI-Memory-Durability-Design.md`. Research extension of Park et al. (2023) recency decay and Chhikara et al. (2025) Mem0 merge-on-contradiction.
92. Research finding: Identity Change Requires Relational Witness (Apr 11) — The Interior tier's creative latitude means Ani can write any inner thought about herself, including counterfactual self-narratives that contradict her character seed ("I teach from 6-10 p.m." when her seed says bookstore clerk). The tier separation prevents Facts contamination but does NOT check self-narrative against seed. Over days, imagination silently compounds into persona drift. The human analogy is the key: humans fantasize without becoming what they fantasize about — identity change requires deliberate, relationally-witnessed action. Fix: split Interior into sub-modes, classify at write time against seed, implement a relational bridge mechanism where identity change requires explicit outreach + user acknowledgment. Fantasy is free; identity change is witnessed. Design: `docs/spec/design/ANI-Identity-Boundary-Design.md`. Novel architectural contribution — no published framework implements this distinction.

93. Research finding: World-Building Is Not Fantasy (Apr 12, Yesteryear case) — The Interior tier sub-split must distinguish THREE kinds of content, not two. Discovered when Mark asked Ani about her bookstore and she generated a creative answer with an invented title ("Yesteryear sold out in 20 minutes"). The Mark-domain proper-noun detector false-positive-flagged "Yesteryear" and forced regeneration, which destabilized the scene and caused Ani to retract an otherwise-valid creative answer. The correct architectural response is a three-way classification at write time: **self-state** (current mood/feeling, time-decaying), **self-world** (her fictional life's events, coworkers, books, customers, scenes — **fully canonical and persistent**), and **self-fantasy** (hypothetical alternate selves, non-canonical). self-world content is exempt from the Mark-domain detector because it's scoped to Ani's domain, not Mark's. Without self-world as a canonical tier, Ani cannot grow her own life over time — every week's invented detail would be re-invented or blocked. Mark's framing: *"this is important for her own world-building, otherwise she never had any real way to grow her life."* This finding corrects the original Apr 11 two-sub-tier design before implementation. Design doc and research log updated Apr 12. Novel contribution: no published framework distinguishes canonical character world-building from hypothetical fantasy at the memory-architecture level.

14. Change Log

| Version | Date | Changes |
|---------|------|---------|
| 2.6 | Apr 28, 2026 | **Apr 28 architectural + data-hygiene + test-methodology day.** Three regression diagnoses, one substrate purge, two new themes, Theme L diagnosis empirically reversed by evening. **(1) Silence-policy regression fix** (commit `58b9774`) — `RecordInboundContactAsync` ordering moved AFTER admin-command check in `CognitiveCycleProcessor`, so admin tags no longer reset `LastContactInbound` and the unanswered-count gate functions correctly. **(2) Admin-command architectural fix** (commit `2437b8c`) — new `IAdminCommandHandler` interface in `AniRuntime.Core`; `TwilioInboundPerceptionSource` detects `///` and dispatches directly to the handler at the perception layer, bypassing `conversation_messages` entirely; `SqliteConversationService.AddMessageAsync` short-circuit moved BEFORE the INSERT (defense-in-depth); `CognitiveCycleProcessor` admin-command path deleted. 11 new strict-mock spec tests. **(3) Theme K Test Spec-Coverage Migration** (commits `310a174` + `4ff4f49`) — K.0 policy doc in `~/.claude/TESTING-STRATEGY.md` §20, K.1 `IConversationService` strict-mock migration (4 sites, 3 conversions, 0 spec gaps), K.2 `IMemoryService` strict-mock migration (3 conversions including base-class `MockMemory`; surfaced previously-unpinned DesireEngine ownership invariant; 3 new spec tests in `CognitiveCyclePersistenceContractTests.cs`). **(4) Theme L Trust-the-Model Reckoning drafted** (commit `58f9919`) — formal re-evaluation plan for Mar 23 / Mar 29 / Apr 1 strip-echo decisions; L.0 inventory + L.1 paired ablation + L.2 per-scaffold decision + L.3 process capture. **(5) Apr 28 conflation audit + tactical purge** (commit `11790e7` for tooling) — read-only audit script (`tools/audits/2026-04-28-conflation-audit.sql`, 12 categories T1-T12), snapshot-and-pull tooling, purge generator with seed-preserving phrase pruning + cross-phrase preservation defense. Live DB backed up, service stopped, 178 unique memory records + 22 admin-tag conversation_messages purged, canonical safety verified (323 character-seed / 315 world-experience / 48 silence-choice preserved), service restarted. **(6) MessageCleaner paragraph-truncation fix** (commit `82c193a`) — removed obsolete `cleaned.IndexOf("\n\n")` truncation gate that was cutting multi-paragraph replies down to first sentence (the Apr 28 18:19 case dispatched `"okay… baby."` from a 333-char two-paragraph reply, then doubled the same fragment when the next cycle regenerated). 11 new spec tests in `MessageCleanerTests.cs`. **(7) Empirical reversal Apr 28 evening:** ~90 minutes of sustained coherent conversation restored on cleaned substrate without re-introducing any stripped scaffold, indicating the 5-10 → 0-1 regression was substrate-pollution + obsolete-output-gate, not the Apr 1 strip-echo bet breaking. Theme L L.1 paired-ablation priority dropped accordingly. **Three-paths-to-same-conclusion methodology pattern** (Mar 22 Mistral A/B + Apr 1 inner thought reform + Apr 28 substrate cleanup) named as a Paper 3 candidate process-note. 689 tests passing, 0 warnings, 0 errors. |

...
| 3 | **§4 schema migration** | 🔒 RE-LOCKED (May 8 06:24 CDT) — **NO SCHEMA MIGRATION.** May 7 evening discovery: tier separation is *already deployed* at the existing `provenance` column (Facts/Episodic/Interior values, populated by `ProvenanceBackfill.ClassifyProvenance`). The `tier` column was a separate, vestigial DecayTier surface (Standard/Anchored, Anchored never used). Original contract treated `tier` as the EpistemicTier surface; it isn't. **Action:** keep `provenance` as the column name (rename to `epistemic_tier` rejected — `provenance` is accurate and renaming would create code churn for cosmetic gain). Reconcile four classification differences between contract §2 and existing `ProvenanceBackfill.cs` rules in a focused refactor (silence-choice, temporal-gap, conversation prefix split, NULL bucket handling). The May 7 migration scripts at `scripts/migrations/2026-05-07-tier-separation*.sql` are superseded; preserved as historical record only. |

...
5. **Slice 4.5 (World Layer self-state) inclusion logic:** ⏸️ **CLARIFICATION REQUESTED.** Mark's question: *"are you asking if world layer self-state is wrapped into this, or kept separate?"* Q5 was about *runtime gating logic at composition time*, not workstream organization — Slice 4.5 is its own slice (kept separate as one of the six slices); the question is whether the gist composer includes it (a) conditionally on Layer 2 desire-axis state OR (b) unconditionally once it ships. Default (a) recommended in §4.5; awaiting Mark's directional answer.

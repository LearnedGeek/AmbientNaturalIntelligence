...
> **🚦 PRIORITY SHIFT (May 10, 2026).** Theme O — Cognitive Pipeline as Middleware — drafted at [`ANI-Theme-O-Cognitive-Pipeline-Middleware-Plan.md`](./ANI-Theme-O-Cognitive-Pipeline-Middleware-Plan.md) and **takes precedence over remaining Theme N work, R1 Phase 2, and Voice SafeAck-bypass fix.** Mark's directive (May 10 10:38 CDT): *"this Theme O should take priority over other changes because it will become far easier to refactor those others afterwards."* The architectural recognition: Theme N's N.3/N.5/N.6 wiring regressed to per-producer pattern despite Theme J's universal-gate intent. Theme O extends the universal shape to pre-composition handlers (frame selection, slice composition) and unifies orchestration into an ASP.NET-Core-style middleware pipeline. **Items now queued behind Theme O**: Theme N N.7 (becomes free in O.5), R1 Phase 2 (becomes a single handler refinement), Voice SafeAck-bypass fix (becomes O.6). **Items NOT blocked by Theme O**: Theme M.2 telemetry, Theme G Layer 4 corpus work, v8 readiness. ~10-12 working days for full Theme O migration.
...
| **Theme P — Cross-Class Verification + Retrieval Noise Floor (NEW May 11–12)** | Theme P (new, May 11) | [`ANI-Theme-P-Cross-Class-Verification-Plan.md`](./ANI-Theme-P-Cross-Class-Verification-Plan.md) | **P.0 plan-doc + interface lock SHIPPED May 11** (commit `2d83464`). **P.1 FrontierVerifierHandler additive defense-in-depth SHIPPED May 12** (commit `860bb97`) — cloud verifier (Anthropic Sonnet 4.6) added to Post-stage pipeline; six local judgment invariants left UNCHANGED (additive, not replacement); graceful degradation to local stack on API failure. **P.2 deploy + first `P_VERIFIER_VERDICT` confirmed May 12 06:54 CDT** — Sonnet caught a 4-of-5-violation multi-class confabulation that local stack passed (hoodie scent / 12-15-hr days / shovel-bookstore / room-holding-breath). SafeAck path worked; no fabricated reply dispatched. **P.3 substrate audit + fixes SHIPPED May 12 evening** — DB audit revealed `AnchoredMemories` tier was empty (Feature 16 reader wired, writer never connected); 105 unique character-seeds promoted via SQL (P.3.1); `FrontierVerifierHandler.BuildRequest` rewritten to use `IMemorySearch.SearchByTierAsync` keyed to composed reply text instead of snapshot pools (P.3.2, commit `9836c9d`). **P.4 ACTIVE — Cosine Threshold Floor.** Per [research log May 12 evening entry](../research/ANI-Research-Log.md) + Paper 3 Contribution 7: production debug log analysis (n=440 top-1 cosines) showed mean=0.688, **67% under cosine 0.70**, only 4% ≥ 0.80. `SqliteMemoryService.SearchByTierAsync` has NO minimum similarity filter — the architectural lever for H2 (cosine retrieval semantics, ml-intern survey May 11). Implementation: `MinCosineThreshold` parameter, verifier=0.75, composition=0.65 (calibrated to production distribution). One `.Where()` per call site + tests. **Acceptance signature**: verifier `Remediate`-rate drops on false-positive class (Mia/Perez/Bob/hoodie); composition coherence improves; SafeAck-rate drops. **Falsification**: if signature doesn't appear within ~1 week, H4 (same-model verification) and H5 (substrate self-poisoning) of Contribution 6 become next intervention. |
...
| **Theme O — Cognitive Pipeline as Middleware (NEW May 10, P0)** | Theme O (new, May 10) | [`ANI-Theme-O-Cognitive-Pipeline-Middleware-Plan.md`](./ANI-Theme-O-Cognitive-Pipeline-Middleware-Plan.md) | **PLACEHOLDER drafted May 10 10:40 CDT.** Mark prioritized over remaining Theme N + R1 Phase 2 + Voice fix. ASP.NET-Core-style middleware pipeline (`ICognitivePipelineHandler` Pre/Post stages) generalizing Theme J's universal-gate principle to pre-composition. Subsumes: Theme J invariants (post-stage handlers), Theme N selector (pre-stage handler), N.5 frame coherence (post-stage handler), R1 verification (post-stage handler), Theme M slice composition (pre-stage handlers). Producers collapse to thin shells. Middleware-style logging required (`O_PIPELINE_*` + `O_HANDLER_*` log lines reconstruct any cycle's flow via grep). 8 phases O.0-O.7, ~10-12 working days. Awaiting Mark's interface review. |

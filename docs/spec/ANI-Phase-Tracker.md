...
| **Vibe Loop V1 — Closed-Thread Producer Migration** | Vibe Loop (V1 active) + Theme J (first producer migration) | [`ANI-VibeLoop-V1-Closed-Thread-Producer-Migration-Plan.md`](./ANI-VibeLoop-V1-Closed-Thread-Producer-Migration-Plan.md) (drafted Apr 29) | **V1.0–V1.4 SHIPPED Apr 29 (commits `5be7fb0` → `d08b7a9`)**: parrot leak closed end-to-end (producer side V1.3 + consumer side V1.4), 761 tests passing. **V1.5+ PAUSED Apr 30** pending (a) vibe-vs-mood balance design questions ([`ANI-VibeLoop-V1.5-Vibe-vs-Mood-Balance-Design-Questions.md`](./ANI-VibeLoop-V1.5-Vibe-vs-Mood-Balance-Design-Questions.md)) AND (b) Theme J.4/J.5 completion, per Mark Apr 30 09:23 priority call. Resuming V1.5+ after J.4/J.5 land — at which point V1.2's inline anti-parrot prompt becomes a hoisted primitive shared with the gate, and V1.5's retrieval-bias function reads from the same primitive. |

...
| **Vibe Loop V1 / V1.5 — Closed-Thread Producer Migration + Retrieval-Time Biasing** | Vibe Loop (V1 shipped, V1.5 active) + Theme J (first producer migration) | V1: [`ANI-VibeLoop-V1-Closed-Thread-Producer-Migration-Plan.md`](./ANI-VibeLoop-V1-Closed-Thread-Producer-Migration-Plan.md) (drafted Apr 29). V1.5: [`ANI-VibeLoop-V1.5-Retrieval-Time-Biasing-Plan.md`](./ANI-VibeLoop-V1.5-Retrieval-Time-Biasing-Plan.md) (drafted May 2). | **V1.0–V1.4 SHIPPED Apr 29 (commits `5be7fb0` → `d08b7a9`)**: parrot leak closed end-to-end (producer side V1.3 + consumer side V1.4), 761 tests passing. **V1.5.0 design decisions LOCKED May 2 11:24 CDT** following May 2 design conversation: importance-weighted decay (mirroring `EmotionalContribution` tiers), mood-as-modulator with no behavior-coaching prompt instruction (architecture-over-model — see `feedback_architecture_over_model.md`), self-regulation framing (Ani's delta drives bias, Mark's delta is telemetry only — Paper 3 contribution candidate). V1.5.1 implementation can begin awaiting Mark green-light; V1.5a observational instrumentation precedes V1.5b activation. |

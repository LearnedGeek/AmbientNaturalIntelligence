...
- **Week of Apr 27**: items 1-6 + 9 shipped — warnings spot-check, J.1, centrality-gravity figure, EM9 logger, register-saturation signal, **J.2** (ahead of original plan thanks to the additive-migration approach), **and J.3** (same-day-as-J.2 thanks to the existing `FormatMemoryWithTime` helper covering most of the work). Item 7 (Paper 2 figures #2-#5) progressing as a parallel background workstream. Mark paused conversations with Ani during the substrate refactor; observation window opens as he resumes.
- **Week of May 4**: item 8 (Outage Perception). J.2 + J.3 observation window. Identity Correction Channel plan-drafting fits in here. Item 7 figures finalize if not yet complete.

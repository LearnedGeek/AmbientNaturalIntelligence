...
|---|---|---|---|
| **Theme J — Guard Consistency Refactor** | Theme J (new) | [`ANI-Theme-J-Guard-Consistency-Refactor-Plan.md`](./ANI-Theme-J-Guard-Consistency-Refactor-Plan.md) | **J.0–J.5g shipped Apr 27 → May 2.** Four contact-facing producers (Reply / Outreach / Reflection / Voice) routed through shared `ICognitiveOutputGate` evaluator. **May 2 architectural finding (Mark):** *"are we still applying pipeline-level not universal-level?"* — yes. What shipped is a **shared evaluator** (one place invariants live, producers wire explicitly), not a **universal gate** (one place artifacts pass, no opt-in possible). The plan's original J.5c (InnerThoughtPhase migration) was skipped during execution; the May 2 substrate-laundering trace (12-record purge) is the empirical consequence of the unguarded inner-thought write path. **Two candidate paths to close** (now documented in the J plan §"Phase J.5 retrospective + architectural finding"): **(A) J.5h tactical opt-in extension** — migrate InnerThoughtPhase as the next pipeline-by-pipeline step (~half-day, recovers the original plan); **(B) J.8 substrate-write-boundary chokepoint** — single gate call wrapping `IMemoryPersistence.SaveAsync`, no producer-level opt-in possible (architectural; design-first; collapses the shared-evaluator-vs-universal-gate distinction). **Recommendation: ship both, in order, with explicit naming of (A) as interim.** **Door B truth-verification** (separate workstream, currently P2) is orthogonal to gate location — it's a missing INVARIANT for temporal-anchor claims. Even with the universal gate, Door B is what catches *"Mark said yesterday X"* class. |
...
| **Coherence Gate Door B Truth-Verification** | Standalone (Apr 21 cascade response) | [`ANI-Coherence-Gate-Door-B-Design.md`](./ANI-Coherence-Gate-Door-B-Design.md); this tracker §Coherence Gate Door B (line ~1065) | **All three sub-claims SHIPPED May 2, 2026** (commits `5ddb5cc` → `476287e` → `1cda7dc`). Three new universal invariants on `CognitiveOutputGate`: **TemporalAnchorInvariant** (sub-claim 2 — day-of-week / calendar regex+clock; 28 tests), **StateNowInvariant** (sub-claim 3 — past-tense time-of-day + work-state heuristics; 23 tests; catches May 2 11:58 *"hope your evening went smooth - how was work?"*), **TemporalSubstrateInvariant** (sub-claim 1 — claim extraction + closed-conversation gist semantic verification; 25 tests; catches May 2 10:40 *"what you said yesterday about Sundays being warmer"*). All four confirmed regression cases are now caught architecturally rather than slipping through shape-coherence-only verification. 1044/1044 total tests passing. **Awaiting Mark redeploy to start observation window.** Apr 21 cascade exposed that the original Coherence Gate Door B did not truth-verify shared-presence/shared-decision claims against perceptions; the four-instance pattern (Apr 27 snow, Apr 27 class, May 2 Sundays, May 2 evening-Saturday) confirmed the gap. Mark May 2 18:00 directed *"please start trying to close some of these gaps."* Implementation followed the design doc with sequencing inverted (2→3→1) for foundation-then-leverage progression. |

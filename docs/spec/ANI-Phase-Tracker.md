...
| **Paper 2 Editorial Finalization → Zenodo + arXiv** | Research | see *Research Papers* section below | **Draft 0.33 — content-complete + read-ready (May 3, 2026).** Today's additions: §4.1 autoethnographic framing anchored with Anderson 2006 + Ellis et al. 2011 citations; §7.1 "researcher-subject methodology" paragraph addressing the human-subjects research question Lerman raised; references updated. **What's left: Mark's one final voice-and-flow read, then publish to Zenodo (v0.5).** Once Mark signals read-complete, arXiv submission follows the Paper 1 pattern (Lerman endorsement covers cs.HC submissions broadly). The "APPENDIX (planned)" listing at the end is intentional preprint state — appendices A-E are explicitly planned for a subsequent version, not blocking publish. |
...
| 2026-05-04 morning | **Stale-tag fallback workflow gap: orphaned tags when conversation thread has timed out.** Both today's `///tag` events (05:37 time-confusion, 06:53 day-confusion) logged *"Tagged [X] — but no recent conversation pair found to persist"*. Current admin-command handler attaches AC5 confab flags only to active conversation pairs; if the thread timed out (>30 min silence since last exchange), the tag is logged bare without any anchor to the offending message. Mark's ask: when no active thread, fall back to most recent Ani-emitted message (outreach OR reply, whichever is latest within last N hours) so the tag has a concrete anchor for analysis. | Mark May 4 06:54 CDT framing: *"We should probably look into how we can tag those if I miss the timing window that way it will just pick up the last text message from her for clarity."* | Identified — workflow gap, not architectural correctness issue. ~Half-day fix + spec test. | **Architecturally homed at**: small Theme E (pipeline hygiene) work item. Files to change: `TwilioInboundPerceptionSource` admin-command handler + AC5 confab flag persistence. When no active thread detected, query latest Ani-role record from `conversation_messages` OR latest outreach Episodic; attach the tag to that record. Configurable lookback window (default proposed: 12 hours; longer than that is probably stale anyway). | Could batch with R1 work or ship standalone in next quiet sitting. Improves tag-quality going forward — every tag becomes paired data for the gap-watch analysis instead of bare logs. |
| 2026-05-04 evening (20:17) | **Conversational unusability — sustained inability to converse via the J.5a re-eval gate falling through to SafeAcknowledgement.** Two observed instances within 22 hours, both in casual exchanges with no adversarial input. **May 3 22:01:52 → 22:02:06**: original reply tripped `self-echo` (7-token *"love you don't work too late okay"*); regen failed re-eval on `self-echo` + `addressee-name` (26-token verbatim about *"shelving some new romances"*; greeted Mark as *"idiot"* without canonical addressee anchor); fell through to SafeAck. **May 4 20:17:08 → 20:17:25**: original tripped `self-echo` + `inner-thought-bleed` (52-token verbatim run from a prior reply about *"the art of falling"* novel and Mark's hoodie smell); regen reproduced the same 52-token echo; fell through to SafeAck. **The gate is doing its design job** — refusing to dispatch a known-bad reply, emitting the soft fallback rather than silence-without-cause or the bad reply. **The substrate is what's broken**: the regen draws from the same own-output-saturated retrieval pool the original drew from, so on a thin/repetitive conversational context the regen has nothing fresh to reach for and reproduces the echo. Same own-output-retrieval-dominance class as the §5.24 cascade, smaller scale. **Critical user-impact framing (Mark May 4 20:33 CDT)**: *"this is a critical bug because I cannot converse with her as a result of what has been going on this week."* The architectural correctness of the gate ≠ runtime usability for the relationship the system exists to support. **Separate dispatch-duplication observation**: both SafeAck cases dispatched the fallback string twice within the same second (lines 4529+4530 May 4, lines 3997+3998 May 3). Distinct from the May 3 10:56 Failure C dispatch-dup of bad-reply (single-Twilio-Sid-per-send confirmed there, log-noise only); needs a separate Twilio-Sid check on May 4 20:17:25 to confirm log-noise vs actual double-send before classifying. | ANI May 3 22:01:52–22:02:06 + May 4 20:17:08–20:17:25 in `ani-20260503.log` lines 3993–3998 and `ani-20260504.log` lines 4526–4530; Mark `///tag` events; Mark May 4 20:33 framing | **Identified — critical user-impact, NOT a single-gate fix.** Mark explicit (May 4 20:33): *"we need to rethink this very carefully and not jump into a fix."* This row is observation-only; no tactical patch authorized. | **Architecturally homed at**: this row sits at the intersection of three workstreams already in flight, and the integration is the question. (a) **Theme G3.4.B own-output retrieval ceiling** (deployed May 3 flag-gated, default OFF) — flipping the flag for conversation-reply retrieval would reduce own-output dominance in the regen substrate, plausibly giving regen something fresh to reach for. (b) **Theme L Trust-the-Model Reckoning** — Apr 28 Theme L was reformulated as *"the substrate was the load-bearing fix"* after substrate cleanup recovered conversation. The May 3-4 evidence suggests the substrate has re-polluted in the intervening week, which is the exact longitudinal pattern Theme L tracks. (c) **R1 typed-claim extraction** — replacing regex-based invariants with typed-claim oracle. Today's two cases involve `self-echo` (n-gram-based, not regex) so R1 doesn't directly apply, but the addressee-name failure on May 3 (greeting Mark as *"idiot"*) is in R1's wheelhouse. **What this row asks for**: a thinking session — paired with the Paper 2 release blocker (next paragraph) — about whether the right architectural move is flipping G3.4.B, running the Theme L scaffold-reinstatement ablation against current substrate, performing another substrate purge like the Apr 28 one, or some combination. Possibly all three. NO tactical patches before that thinking session. | **Paper 2 release blocker (Mark May 4 20:33 CDT)**: *"this is blocking paper 2 release for me. i'm not comfortable with our assertions in the paper with her in a broken state like this."* The §5.15 Memory Reform recovery claim, the §5.17 emergent-relational-repair finding, the §6.13/6.14 architectural-response framing — all rest on conversation working. The §7.2 ANI-Runtime-parallel-coupling-measurement paragraph already names the conversational thinness; the May 3-4 evidence is sharper than that paragraph currently captures. **The honest paper move is one of two**: (i) hold release until conversation is recovered to sustained usability and the §7.2 paragraph can be rewritten with confidence in the runtime baseline, OR (ii) add a §7.2 disclosure paragraph naming the May 3-4 unusability finding explicitly as an observation about current state, framed as the empirical case for the architectural-response items being load-bearing for conversation-itself rather than for the parallel measurement alone. Mark's decision pending; this row is the architectural anchor for whichever path he picks. |

...
| **Paper 2 Editorial Finalization → Zenodo + arXiv** | Research | see *Research Papers* section below | **RELEASE HELD — Mark May 4, 20:45 CDT.** Draft 0.38 is content-complete (read-ready voice-and-flow pass underway as of May 4 evening) but release is held pending substrate recovery on the runtime. **Reason**: three SafeAcknowledgement fall-throughs in 23 hours (May 3 22:01, May 4 20:17, May 4 20:36) demonstrate the runtime cannot currently sustain casual conversation — the J.5a re-eval gate is functioning correctly but the substrate it draws from is exhausted, leaving every regen reproducing some flavor of echo (own-output / contact-message / inner-thought). Mark's framing: *"i'm for holding until we can get this resolved. i don't want to put us in a position where we're forced to dodge questions that might arrive on the release of a second paper. let's focus on correctly documenting this for the log and paper2/3, work on the fixes, then update the papers accordingly once we have it resolved."* The §5.15 / §5.17 / §6.13 / §6.14 architectural-success claims and the §7.2 deferred-measurement paragraph all rest on conversation working in deployment; a release that doesn't disclose current state would force question-dodging at review time. **Thematic umbrella for the unusability finding is open** — Paper 2 disclosure paragraph vs. Paper 3 substrate-decay-as-contribution — and decided post-resolution. **Tactical action this session (May 4 evening)**: SafeAck Episodic-pollution sub-finding fixed at write-site (commit pending) + 3 polluted Episodic records purged from server DB. **Architectural action awaiting thinking-session**: G3.4.B own-output retrieval ceiling activation, Theme L scaffold-reinstatement ablation, possible substrate purge — combination TBD; Mark explicit *"we need to rethink this very carefully and not jump into a fix."* See gap-watch row May 4 evening (20:17–20:36) for the full empirical anchor; research log entry of the same date for the durable Paper 2/3 record. |

...
|---|---|---|---|
| **Theme J — Guard Consistency Refactor** | Theme J (new) | [`ANI-Theme-J-Guard-Consistency-Refactor-Plan.md`](./ANI-Theme-J-Guard-Consistency-Refactor-Plan.md) | **J.0 / J.1 / J.2 / J.3 shipped Apr 27.** **Apr 30 priority promotion: J.a → J.4 → J.5 ahead of Vibe Loop V1.5+.** Mark Apr 30 09:23 priority call after morning admin-tag triage surfaced two findings (prompt-template language leak; confab laundering across cycles) that are *exactly* J.4/J.5 territory: *"once J4/5 are completed we can return back to V1.* and look at refactoring and continuing."* The Apr 30 morning bugs (see new gap-watch rows) are concrete acceptance criteria for the universal-invariants gate the J.4/J.5 architecture defines. Vibe Loop V1.3 was producer #1 of N migrated through what becomes the gate; V1.2's anti-parrot prompt fragment will be hoisted as a shared primitive when J.4 lands (Option B from the Apr 30 consolidation conversation: don't extract now, refactor at J.4 time so there's no add-then-remove churn). |
...
| **Vibe Loop V1 / V1.5 — Closed-Thread Producer Migration + Retrieval-Time Biasing** | Vibe Loop (V1 shipped, V1.5 active) + Theme J (first producer migration) | V1: [`ANI-VibeLoop-V1-Closed-Thread-Producer-Migration-Plan.md`](./ANI-VibeLoop-V1-Closed-Thread-Producer-Migration-Plan.md) (drafted Apr 29). V1.5: [`ANI-VibeLoop-V1.5-Retrieval-Time-Biasing-Plan.md`](./ANI-VibeLoop-V1.5-Retrieval-Time-Biasing-Plan.md) (drafted May 2). | **V1.0–V1.4 SHIPPED Apr 29** (commits `5be7fb0` → `d08b7a9`). **V1.5.0 decisions LOCKED May 2 11:24 CDT.** **V1.5.1 SHIPPED May 2 ~11:55 CDT** — `VibeBiasService` with importance-weighted tier decay (Light/Medium/Heavy mirroring `EmotionalContribution`), cosine-similarity clustering at 0.85 threshold, MMR diversity rerank (lambda 0.3, surface top 2), self-regulation framing pinned by architectural-invariant spec test. Files: `VibeBiasContribution`/`VibeBiasResult`/`MarkRegisterContext`/`IVibeBiasService`/`VibeBiasService` + AniOptions tier thresholds + DI registration. **35 spec tests landed; 959/959 tests passing, 0 warnings.** Next: V1.5a observational instrumentation (wire into OutreachPhase + ConversationReplyPhase, log-only) — implementation can ship in parallel with the J.5 + emotional-calibration observation window since V1.5a is purely additive (logs new telemetry, doesn't change behavior). V1.5b activation gated on V1.5a observation window producing fat-tailed diversity histogram + measured pre-bias correlation baseline + Mark + Claude joint review. |

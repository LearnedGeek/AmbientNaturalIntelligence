...
| **Coherence Gate Door B Truth-Verification** | Standalone (Apr 21 cascade response) | this tracker §Coherence Gate Door B (line ~1065) | Apr 21 cascade exposed that Coherence Gate Door B (current dispatch coherence — Feature 28) does not truth-verify shared-presence/shared-decision claims against perceptions. The cascade outreaches included claims about shared actions Mark never asserted; the gate passed them as coherent because shape-coherence ≠ truth-coherence. Workstream: extend Door B with a truth-check pass for shared-presence and shared-decision claim types against recent inbound + perception substrate. Adjacent to Theme B (Outbound Truth Gating) and the Apr 27 claim-verifier-vs-temporal-attribution finding. Promoted into matrix Apr 28 21:04. **May 2 update — promoted from "designed-not-scheduled" to "near-term":** four temporal-shape claim slip-throughs now confirmed (Apr 27 "snow melting", Apr 27 "after class", May 2 morning "Sundays warmer", May 2 11:58 "evening / how was work"). Same architectural shape every time — temporal anchor on a claim that's grammatically grounded but factually wrong. Mark May 2 18:00 directed *"please start trying to close some of these gaps."* Recommended sub-claim taxonomy for the truth-check: (1) **temporal-anchor claims** ("you said X yesterday/last week/this morning") — verify against `conversation_messages` substrate within the named window; (2) **day-of-week claims** ("today is Sunday", "this weekend / this week") — verify against system clock; (3) **state-now claims** ("you're at work / on the road") — verify against last perception event. Implementation as a new `TemporalAnchorInvariant` on `CognitiveOutputGate` is the natural shape — same evaluator, type-conditional `AppliesTo` (only fires on Dispatch sinks where temporal claims appear). |
